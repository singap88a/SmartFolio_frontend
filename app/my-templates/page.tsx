...
export default function MyTemplatesPage() {
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete your portfolio?')) return;
    setIsDeleting(true);
    try {
      await fetchApi('/portfolio', { method: 'DELETE' });
      setPortfolio(null);
    } catch {
    ...
    <div className="flex-1 flex flex-col h-full bg-[#0B0F19] overflow-y-auto custom-scrollbar p-6 lg:p-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          ...
          <p className="text-slate-400 text-sm">View and manage your active portfolio template.</p>
        </div>
        <Link href="/templates">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {/* Portfolio Card */}
          <div className="group bg-[#0F121E] border border-[#1E2336] rounded-3xl overflow-hidden hover:border-[#5A4BFF]/40 transition-all duration-300 hover:shadow-[0_0_30px_rgba(90,75,255,0.08)]">
            ...
            <div className="p-5">
              <div className="mb-4">
                <h3 className="text-lg font-bold text-white mb-1">
                  {portfolio?.data?.name || user?.username || 'My Portfolio'}
                </h3>
                <p className="text-sm text-slate-400 truncate">
                  {portfolio?.data?.jobTitle || 'Portfolio'}
                </p>
              </div>
